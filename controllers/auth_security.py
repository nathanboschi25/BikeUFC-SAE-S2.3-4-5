...
def auth_login_post():
    ...
    tuple_select = (login,login)
    sql = "SELECT * from utilisateur where login like %s or email like %s;"
...
def auth_signup_post():
    ...
    sql = "INSERT INTO utilisateur(login, email, password, role) VALUE (%s, %s, %s, %s)"
    mycursor.execute(sql, tuple_insert)
    get_db().commit()
    sql = "SELECT LAST_INSERT_ID() as last_insert_id FROM utilisateur"
